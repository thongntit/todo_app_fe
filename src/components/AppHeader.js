    ...
    return (
        <Grid container className={classes.header}>
            <Grid className={classes.title} item xs={2}>
                <Typography className={classes.titleText} variant="h6">Todoist</Typography>
            </Grid>
            <Grid item xs={8}></Grid>
            <Grid item xs={2} className={classes.pCon}>
                <IconButton onClick={handleMenu}>
                    {user && user.profile.photoURL ? <img alt="Profile" style={{ height: 30, width: 30, borderRadius: 15 }} src={user.profile.photoURL} /> : <AccountCircleIcon />}
                </IconButton>
                <Menu
                    id="menu-appbar"
                    // anchorEl={anchorEl}
